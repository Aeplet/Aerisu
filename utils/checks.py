...
def is_staff(role: str):
    async def predicate(ctx: commands.Context):
        if check_staff(ctx.bot, role, ctx.author.id) or (ctx.guild and ctx.author == ctx.guild.owner) or (ctx.author.id in ctx.bot.owner_ids):
            return True
...
def is_staff_app(role: str):
    async def predicate(interaction: discord.Interaction) -> bool:
        if (interaction.guild and interaction.user == interaction.guild.owner) or check_staff(interaction.client, role, interaction.user.id) or (interaction.user.id in interaction.client.owner_ids):
            return True
